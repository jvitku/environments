...
//import org.hanns.environments.discrete.learningAlgorithm.models.qMatrix.FinalQMatrix;

/**
 * 
 * GridWorldOld simulation engine.
 * 
 * The world is represented as a 2D matrix containing reward values.
 * 
 * No obstacles are allowed, agent is not allowed to go out of the map.
 * 
 * @author Jaroslav Vitku
 */
public class GridWorldOld {
	/**
	...
	public static String visqm(FinalQMatrix<Double> q, int what){
		...
		for(int i=dimsizes[1]-1; i>=0; i--){
			...
			for(int j=0; j<dimsizes[0]; j++){
				...
				best = GridWorldOld.getMaxInd(actionvals);
				if(what==0){
					if(best<0){
						line = line+"\t"+best;
					}else
